...
class PixelFontSizeTest {
    ...
    fun `every size lands on the eight-pixel grid`() {
        densities.forEach { density ->
            steps.forEach { step ->
                val (size, leading) = snapFontPixels(13f * density, step)
    ...
    fun `the three sizes stay distinct and ordered`() {
        densities.forEach { density ->
            val small = snapFontPixels(13f * density, -1).first
            val body = snapFontPixels(13f * density, 0).first
            val large = snapFontPixels(13f * density, 2).first
